...
connect(NetWork, SIGNAL(progresseIsVisible(bool)), progress, SLOT(setVisible(bool)));
       ...
       QWidget *widgetMedia=new QWidget;

       QHBoxLayout *horizontalLayoutMedia = new QHBoxLayout(widgetMedia);
       horizontalLayoutMedia->addWidget(slider);
       horizontalLayoutMedia->addWidget(progress);
       ...
       if(m_standardTool==false){
           ...
           sceenbase->toolbarMedia->addSeparator();
//           sceenbase->toolbarMedia->addWidget(slider);
//            sceenbase->toolbarMedia->addWidget(volume);
           sceenbase->toolbarMedia->addWidget(widgetMedia);
           sceenbase->toolbarMedia->addAction(ui->actionPlayPause);
           sceenbase->toolbarMedia->addAction(ui->actionStopMedia);
...
//                 ui->toolBar->addWidget(volume);
                  ui->toolBar->addWidget(widgetMedia);
...
void MainWindow::stupActionsShortcuts()
{
    ...
    ui->actionPlayPause->setShortcut(QKeySequence(listShortcut.at(12)));
...
//    qDebug()<<trUtf8("Page")+" : "+QString::number(m_curentSafha);
//    qDebug()<<trUtf8("Page Left")+" : " +QString::number(m_curentSafhaLeft);
//    qDebug()<<trUtf8("Page Right")+" : " +QString::number(m_curentSafhaRight);
...
void MainWindow::moveSelection(int top,int hight,int x1,int x2)
{
    ...
    if(m_autoRepeatOne==false){     //اذا كان وضع القراءة وتم تحريك التحديد
        ...
        if(isPlay==true){
            //    qDebug()<<"her isPlay=true  to mediaplay";
            mediaPlay();
        }
    }
}
...
if (dlg->exec() == QDialog::Accepted){
    ...
    m_saveMediaLoaded=dlg->getSaveMediaLoaded();
...
bool MainWindow::mediaPlayInLocale(QString dirPath)
{
 //    qDebug()<<"=============mediaPlayInLocale()==================";
  ...
  //   qDebug()<<"mediaPlayInLocale"+fileName;
    if(QFile::exists(fileName)){
    qDebug()<<"curent media file name :"+fileName;
        if(m_MediaObject.currentSource().fileName()!=fileName){
            m_MediaObject.clearQueue();
            //في حالة السورة ليست الانفال و الاية هي الاولى وخيار قراءة البسملة مفعل
            //سيضيف البسملة و الاية
            if (m_aya==1&&m_sura!=9&&m_sura!=1&&m_reciteBasmala==true){
                m_MediaObject.clear();
                        QString besmala;
#if defined(Q_OS_WIN)
                besmala="/bismillah.mp3";
#else
                besmala="/bismillah.ogg"   ;
#endif
                m_MediaObject.enqueue(QString(m_appPath+besmala));
                m_MediaObject.enqueue(fileName);

            }else{
                qDebug()<<"mediaPlayInLocale setCurrentSource : "+fileName;
                m_MediaObject.setCurrentSource(fileName);
            }
        }
   //     qDebug()<<"mediaPlayInLocale m_MediaObject.play()";
        m_MediaObject.play();
        return true;
    }else{
        qDebug()<<"media no existe :"+fileName;
        return false;
    }
}
...
void MainWindow::mediaPlayInNetwork()
{
    ...
    if(m_saveMediaLoaded==false){
        ...
        if(url.isValid()){
            qDebug()<<"Curent media file name url:"+url.toString();
  ...
  //  slider->hide();
    if(NetWork->downloadFile(m_soundUrl+"/"+m_soundName+".mp3",destFile)==true){
        m_MediaObject.setCurrentSource(fileurl);
        ...
        qDebug()<<"this file network not valid : "<< m_soundUrl+"/"+m_soundName+".mp3";
...
//    if (listPath.count()>0){
//        curentAyaPath = listPath.at(0);
//    }else{
//        curentAyaPath=m_soundName+".mp3";
//    }
//    QString appFileName=m_appPath+"/"+m_soundPath+"/"+curentAyaPath;
//    if(QFile::exists(appFileName)){
//        qDebug()<<"curent media file name shar :"+appFileName;
...
//    if (listUser.count()>0){
//        curentAyaUser = listUser.at(0);
//    }else{
//        curentAyaUser=m_soundName+".mp3";
//    }
...
//    QString url(destFile+m_soundName+".mp3");
//progress->show();
//slider->hide();
//    if(NetWork->downloadFile(m_soundUrl+"/"+m_soundName+".mp3",destFile)==true){
//         m_MediaObject.setCurrentSource(url);
...
//    }else{
//        qDebug()<<"this file network not valid : "<< m_soundUrl+"/"+m_soundName+".mp3";
//        qDebug()<<"this destfile network"<<url;
//        progress->hide();
//        slider->show();
//        m_MediaObject.stop();
//        return;
...
{
//    qDebug()<<"============mediaFinished";
//    qDebug()<<"m_plaOne : "<<m_plaOne;
//    qDebug()<<"m_autoPlayNext : "<<m_autoPlayNext;
//     qDebug()<<"m_autoRepeatSura : "<<m_autoRepeatSura;
//     qDebug()<<"m_autoRepeatPage : "<<m_autoRepeatPage;
//       qDebug()<<"m_autoRepeatOne : "<<m_autoRepeatOne;
//      qDebug()<<"m_autoRepeatFromTo : "<<m_autoRepeatFromTo;
